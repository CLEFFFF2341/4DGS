...
from .cache import build_k0, build_k1, build_k2, build_k4
...
def main() -> None:
    ...
    subparsers.add_parser("build-k2")
    ...
    if args.command == "manifest":
        ...
        with gpu_lock():
            ...
            elif args.command == "build-k2":
                samples = json.loads((MANIFEST_ROOT / "samples.json").read_text(encoding="utf-8"))
                splits = json.loads((MANIFEST_ROOT / "splits.json").read_text(encoding="utf-8"))
                result = build_k2(adapter, scene, splits["c4"], samples["T24"], samples["statistics_resolution"], ROOT / "research_cache" / "cut_roasted_beef")

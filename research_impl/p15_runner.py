...
from .methods.p15 import correctness_checks, encode_decode, load_compact_bundle, save_compact_bundle
...
def reload_check(expected, restored, scene, camera_name: str) -> dict:
    cameras = [select_camera(scene, camera_name, 149)]
    fractional = copy.copy(select_camera(scene, camera_name, 0))
    fractional.timestamp = 0.5
    cameras.append(fractional)
    differences = [(render_one(expected, camera) - render_one(restored, camera)).abs() for camera in cameras]
    result = {
        ...
        "render_max_abs": max(float(value.max()) for value in differences),
        "render_mean_abs": sum(float(value.mean()) for value in differences) / len(differences),
        "timestamps": [149, 0.5],
    }
...
def main() -> None:
    config_path = ROOT / "configs" / "research" / "p15.json"
    ...
    with gpu_lock():
        ...
        k4_meta = build_k4(reference, [0, 74, 149, 224, 299], ROOT / "research_cache" / "cut_roasted_beef")
        k4 = torch.load(k4_meta["path"], map_location="cpu")
        length_scale = float(k4["length_scale"])
        smoke = {}
        for rule in config["rules"]:
            smoke_started = time.perf_counter()
            _, _, _, smoke_diagnostics = encode_decode(reference, rule, length_scale, config["target_variable_fraction"], smoke_points=128)
            smoke[rule] = {**smoke_diagnostics, "seconds": time.perf_counter() - smoke_started}
        dump(base / "selection_smoke_128.json", smoke)
        for rule in config["rules"]:
            run_started = time.perf_counter()
            output = base / rule
            output.mkdir(parents=True, exist_ok=True)
            dump(output / "resolved_config.json", {**config, "rule": rule, "k4_key": k4_meta["key"], "length_scale": length_scale})
            (output / "commands.txt").write_text("python -m research_impl.p15_runner\n", encoding="utf-8")
            transform_started = time.perf_counter()
            transformed, encoded, resources = encode_decode(reference, rule, length_scale, config["target_variable_fraction"])
            timing = {"selection_and_decode_seconds": time.perf_counter() - transform_started}
            dump(output / "budget_trajectory.json", resources.pop("budget_trajectory"))
            serialization_started = time.perf_counter()
            torch.save(encoded, output / "encoded_knots.pt")
            ...
            render_smoke_started = time.perf_counter()
            render_smoke = smoke_render(transformed, scene, [splits["development"][0]], [0, 149], output / "smoke")
            timing["render_smoke_seconds"] = time.perf_counter() - render_smoke_started
            ...
            dump(
                ...
                {
                    "rule": rule,
                    "source_tensors": ["_xyz_motion", "_rotation_motion"],
                    "position_shape": list(reference.model._xyz_motion.shape),
                    "rotation_shape": list(reference.model._rotation_motion.shape),
                    "stable_ids_preserved": True,
                },
            )
            dump(output / "status.json", {"status": "COMPLETED", "rule": rule})
            rows.append({"rule": rule, **summary, **{key: value for key, value in resources.items() if key not in {"render_smoke", "latency", "bundle_reload", "compact_bundle_reload"}}, "timing": timing})
